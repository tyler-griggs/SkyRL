...
class MegatronPolicyWorkerBase(MegatronWorker, PolicyWorkerBase):
    ...
    def _normalize_mini_batch_size(self):
        ...
        Sets policy_mini_batch_size_per_gpu for the deprecated ppo_train method.
        The new forward_backward method doesn't use this, but we keep it for
        backward compatibility with existing scripts that call ppo_train directly.
        """
        super()._normalize_mini_batch_size()  # Sets _micro_batches_accumulated

        # Megatron-specific: compute mini batch size per GPU (for deprecated ppo_train)
