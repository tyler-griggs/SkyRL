...
def load_checkpoint(checkpoint_dir: str | os.PathLike, config: PretrainedConfig, model: nnx.Module) -> None:
    ...
    for path, param in model_params:
        ...
        if "lora_A" in path or "lora_B" in path or "lora_scaling" in path or "lora_ranks" in path:
            continue
        if "experts" in path:
            # Load expert weights directly into sharded array to avoid OOM
            # Use numpy.stack on CPU, then jax.device_put with existing sharding
            # This avoids materializing the full array on each GPU
            expert_tensors_np = np.stack([tensors[get_expert_key(path, i)].T for i in range(config.num_experts)], axis=0)
            tensors[key] = jax.device_put(expert_tensors_np, param.sharding)
        else:
